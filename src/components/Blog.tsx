...
const Blog = () => {
  ...
  return (
    <section ref={sectionRef} id="blog" className="py-20 bg-gradient-to-br from-gray-50 to-blue-50 dark:from-black dark:to-gray-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`text-center mb-16 transition-transform duration-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-6">
            Latest <span className="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent dark:from-blue-400 dark:to-purple-400">Insights</span>
          </h2>
        ...
        {filteredPosts.find(post => post.featured) && (
          <div className={`mb-16 transition-transform duration-300 delay-100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            {filteredPosts.filter(post => post.featured).map((post, index) => (
              <div key={index} className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl overflow-hidden lg:flex hover:shadow-2xl hover:shadow-blue-500/10 hover:-translate-y-1 transition-all duration-500 group cursor-pointer">
                ...
                <div className="lg:w-1/2 p-8 lg:p-12 flex flex-col justify-center">
                  <span className="bg-blue-100 text-blue-800 dark:bg-gray-800 dark:text-blue-300 px-3 py-1 rounded-full text-sm font-medium mb-4 self-start group-hover:bg-purple-100 dark:group-hover:bg-purple-900 group-hover:text-purple-800 dark:group-hover:text-purple-300 transition-colors duration-300">
                    Featured
                  </span>
                  <h3 className="text-3xl font-bold text-gray-900 dark:text-white mb-4 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-300">{post.title}</h3>
                  <p className="text-gray-600 dark:text-gray-300 mb-6 leading-relaxed group-hover:text-gray-700 dark:group-hover:text-gray-200 transition-colors duration-300">{post.excerpt}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {filteredPosts.filter(post => !post.featured).map((post, index) => (
            <article 
              key={index}
              className={`bg-white dark:bg-gray-900 rounded-xl shadow-lg hover:shadow-xl hover:shadow-blue-500/10 transition-transform duration-300 overflow-hidden group hover:-translate-y-2 hover:scale-105 cursor-pointer ${
        ...
        <div className={`text-center mt-16 transition-all duration-1000 delay-1200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-800 dark:to-purple-800 rounded-2xl p-8 text-white hover:shadow-2xl hover:shadow-blue-500/25 hover:-translate-y-1 transition-all duration-500 relative overflow-hidden group">
            ...
            <p className="text-xl mb-6 text-blue-100 dark:text-blue-200">Subscribe to our newsletter for the latest insights and industry trends.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center max-w-md mx-auto">
              ...
              />
              <button className="bg-white dark:bg-gray-900 text-blue-600 dark:text-blue-400 px-6 py-3 rounded-lg font-semibold hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 hover:scale-105 whitespace-nowrap relative overflow-hidden group/sub">
                <div className="absolute inset-0 bg-blue-50 dark:bg-gray-800 opacity-0 group-hover/sub:opacity-100 transition-opacity duration-300"></div>
