      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`text-center mb-16 transition-transform duration-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">
            About <span className="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">CodeBlend</span>
          </h2>
        ...
        {/* Stats Section */}
        <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 mb-12 sm:mb-20 transition-transform duration-300 delay-100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          {stats.map((stat, index) => (
            <div 
              key={index} 
              className={`text-center p-6 bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl hover:shadow-xl hover:shadow-blue-500/10 hover:-translate-y-2 hover:scale-105 transition-transform duration-300 cursor-pointer group ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
              style={{ transitionDelay: `${index * 60 + 100}ms` }}
            >
              <stat.icon className="h-8 w-8 text-blue-600 mx-auto mb-4 group-hover:scale-110 group-hover:text-purple-600 transition-all duration-300" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mb-12 sm:mb-16">
          {values.map((value, index) => (
            <div 
              key={index}
              className={`bg-white p-8 rounded-xl shadow-lg hover:shadow-xl hover:shadow-blue-500/10 transition-transform duration-300 hover:-translate-y-2 hover:scale-105 group cursor-pointer ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
              style={{ transitionDelay: `${index * 80 + 200}ms` }}
            >
              <div className="bg-gradient-to-r from-blue-600 to-purple-600 w-12 h-12 rounded-lg flex items-center justify-center mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300">
        ...
        <div className={`text-center transition-transform duration-300 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          >
            {skills.map((skill, index) => (
              <div
                key={index}
                className={`inline-block flex-shrink-0 bg-gradient-to-r from-blue-100 to-purple-100 px-12 py-10 rounded-2xl font-semibold text-gray-800 text-xl hover:shadow-xl hover:shadow-blue-500/20 hover:-translate-y-1 hover:scale-105 transition-transform duration-300 cursor-pointer text-center align-top ${
